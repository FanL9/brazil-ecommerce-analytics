...
Grain: one row per observed natural month containing positive-payment
delivered orders. Payments are filtered and aggregated to order_id before any
order join. All five KPIs use this same paid-delivered order scope.
*/
...
first_purchase AS (
    ...
    FROM paid_delivered_orders AS o
...
monthly_paid_kpi AS (
    SELECT
        ...
        SUM(o.order_payment_amount) AS gmv,
...
SELECT
    ...
    CASE
        WHEN m.order_count = 0 THEN NULL
        ELSE 1.0 * m.gmv / m.order_count
    END AS average_order_value,
    COALESCE(n.new_users, 0) AS new_users,
    m.active_users AS active_users
FROM monthly_paid_kpi AS m
